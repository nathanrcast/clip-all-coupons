...
| **Target Circle** | Target | clicks Save/Apply on Deals ("Coupons to apply") | stop only if Target shows a maxed/save-limit modal |
...
usage from Target's loyalty API; offer-group enumeration is skipped (live 502). Stops only if
an on-page save-limit modal appears (API "75 slots" is not a hard stop).
