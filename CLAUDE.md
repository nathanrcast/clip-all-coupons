# safeway-clipper — repo facts

Clip all Safeway / Albertsons "for U" coupons at once. The tool runs entirely client-side in the
...
## Confirmed Safeway for-U API (probed 2026-06-15, store 1487, Firefox)
- **Session globals (page MAIN world):** token `window.AB.userInfo.SWY_SHOP_TOKEN`; `storeId` from
  ...
  same public web-client values Safeway's own site uses — not user secrets.
...
4. **Auto-update URLs** in `safeway-clip-all.user.js` (`@downloadURL`/`@updateURL`) point at this
   repo's GitHub raw path — keep them in sync if the repo/branch moves.

## Files
- `browser/safeway-clip-all.user.js` — userscript (floating button + menu command). Live tool.
